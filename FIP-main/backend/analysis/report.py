...
from backend.schema import RCAResult, ForensicEvent, Case, Note, Upload
...
def _suspicious_events_html(events: list[ForensicEvent], result: RCAResult) -> str:
    ...
    """
    high_risk_users = {
        s.user for s in (result.ml_anomaly_scores or [])
...
def _entity_involvement_html(events: list[ForensicEvent], result: RCAResult) -> str:
    ...
    ml_risk: dict[str, str] = {
        s.user: s.risk_level for s in (result.ml_anomaly_scores or [])
    }
    ...
    if user_cards:
        sections.append(
            ...
            f'User Accounts ({len(user_total)})</p>'
...
<div class="two">
  ...
  <div class="card"><h3>Initial Access Vector</h3>
    ...
    narrative_section = _sec("Detailed Narrative", f'<p class="narr">{result.narrative}</p>')
...
def _ml_anomaly_chart_svg(ml_scores: list) -> str:
    ...
    for i, s in enumerate(scores):
        ...
        label = s.user[:18] + ("…" if len(s.user) > 18 else "")
...
def generate_case_report(
    ...
    Section order:
      Cover → Stats → Severity gauge → Executive summary → Entity involvement
      → RCA → Narrative → Event density → Kill chain → Pivot chain
      → Anomalous events → Suspicious events → MITRE → ML anomaly scores
      → IOCs → Detection rules → Remediation plan → Evidence manifest
      → Analyst notes
    """
    sev = severity_label(result.severity_score)
    now = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")
    ...
    status_cls = {
        "active": "status-active", "closed": "status-closed", "archived": "status-archived",
    }.get(case.status, "status-closed")
...
<div class="case-cover">
  ...
  <h1>{case.title}</h1>
  <div class="case-meta">
    <div class="cm">Case ID<span style="font-family:monospace;font-size:12px">{case.case_id}</span></div>
    <div class="cm">Status<span class="{status_cls}">{case.status.upper()}</span></div>
    <div class="cm">Created<span>{case.created_at.strftime("%Y-%m-%d")}</span></div>
    <div class="cm">Analyst<span>{case.created_by or "unknown"}</span></div>
    <div class="cm">Report Generated<span>{now}</span></div>
    <div class="cm">Severity<span><span class="badge {sev}">{sev}</span></span></div>
  </div>
  {f'<p style="margin-top:18px;font-size:13px;color:#94a3b8">{case.description}</p>' if case.description else ''}
</div>"""
...
<div class="sec">
  ...
  <div class="stat-row">
    <div class="stat-box"><div class="sv">{result.analyzed_event_count:,}</div><div class="sl">Total Events</div></div>
    <div class="stat-box"><div class="sv">{len(hosts)}</div><div class="sl">Hosts</div></div>
    <div class="stat-box"><div class="sv">{len(users)}</div><div class="sl">User Accounts</div></div>
    <div class="stat-box"><div class="sv">{result.windows_analyzed}</div><div class="sl">Analyses Run</div></div>
  </div>
  <div class="stat-row">
    <div class="stat-box"><div class="sv">{len(result.mitre_techniques)}</div><div class="sl">MITRE Techniques</div></div>
    <div class="stat-box"><div class="sv">{len(result.iocs)}</div><div class="sl">IOCs</div></div>
    <div class="stat-box"><div class="sv" style="color:{score_color}">{result.severity_score}</div><div class="sl">Severity /100</div></div>
    <div class="stat-box"><div class="sv">{result.confidence.upper()}</div><div class="sl">Confidence</div></div>
  </div>
  <p style="font-size:12px;color:#94a3b8">Timeline: {t_start} &rarr; {t_end}</p>
</div>"""
...
<div class="sec">
  ...
  <div class="gauge-wrap">
    ...
    <div class="gauge-stats">
      ...
      <div class="gstat"><div class="label">MITRE Techniques</div>
        <div class="value">{len(result.mitre_techniques)}</div></div>
      <div class="gstat"><div class="label">Indicators of Compromise</div>
        <div class="value">{len(result.iocs)}</div></div>
    </div>
  </div>
</div>"""
    ...
    if result.patient_zero_candidate or result.initial_access_vector:
        rca_section = _sec("Root Cause Analysis", f"""
<div class="two">
  <div class="card"><h3>Patient Zero Candidate</h3>
    <p>{result.patient_zero_candidate or "Not identified"}</p></div>
  <div class="card"><h3>Initial Access Vector</h3>
    ...
    narrative_section = (
        _sec("Investigation Narrative", f'<p class="narr">{result.narrative}</p>')
        if result.narrative else ""
    )

    density_svg = _event_density_chart_svg(events)
    density_section = (
        _sec("Event Activity Over Time", f'<div class="chart-wrap">{density_svg}</div>')
        if density_svg else ""
    )
    ...
    if notes:
        ...
        for n in notes:
            ...
            pin_badge = '<span style="color:#1d4ed8;font-weight:700"> 📌</span>' if n.is_pinned else ""
            ts = n.created_at.strftime("%Y-%m-%d %H:%M")
            note_cards += (
                f'<div class="note-card{pinned_cls}">'
                f'<div class="note-meta">{n.author} · {ts}{pin_badge}</div>'
                f'<div class="note-body">{n.content}</div>'
                f'</div>'
            )
        notes_html = _sec(f"Analyst Notes ({len(notes)})", note_cards)

    evidence_html = ""
    if uploads:
        rows_html = ""
        for u in uploads:
            ts = u.uploaded_at.strftime("%Y-%m-%d %H:%M") if u.uploaded_at else "—"
            rows_html += (
                f'<tr><td style="font-family:monospace">{u.filename}</td>'
                f'<td>{u.event_count:,}</td>'
                f'<td>{u.uploaded_by or "—"}</td>'
                f'<td>{ts}</td>'
                f'<td style="font-family:monospace;font-size:10px;color:#94a3b8">'
                f'{(u.file_hash or "")[:16]}{"…" if u.file_hash else "—"}</td></tr>'
            )
        evidence_html = _sec(
            f"Evidence Sources ({len(uploads)} file{'s' if len(uploads) != 1 else ''} uploaded)",
            f'<table><thead><tr><th>Filename</th><th>Events</th><th>Uploaded By</th>'
            f'<th>Upload Time</th><th>SHA-256 (partial)</th></tr></thead>'
            f'<tbody>{rows_html}</tbody></table>',
        )

    footer = (
        f'<div class="ftr"><span>Forensic Intelligence Pipeline v4 — Case Report</span>'
        f'<span>{now}</span></div>'
    )
    ...
    return (
        f'<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">'
        f'<title>Case Report — {case.title}</title>'
        f'<style>{combined_css}</style></head><body><div class="page">{body}</div></body></html>'
    )
