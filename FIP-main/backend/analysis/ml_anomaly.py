...
Workflow:
  ...
  2. run_full_analysis() calls score_all_users() on every analysis run.
...
def _extract_features(events: list[ForensicEvent]) -> np.ndarray:
    """
    Return a (1, 10) feature vector for one user's session.
    ...
    # A slow start followed by a burst is a common attack pattern.
    mid = max(total // 2, 1)
    ...
    # ── Authentication features (graceful if no auth events present) ──────────
    ...
    # ── Process / command features (graceful if 4688 absent) ─────────────────
...
def _heuristic_score(events: list[ForensicEvent]) -> UserAnomalyScore:
    ...
    # Only flag off-hours when it dominates the session (>80% of activity)
    # A ratio of 0.3 is too sensitive — night-owl admins and scheduled tasks fire it constantly.
    ...
    # Require more than 2 admin-tool events; a single whoami doesn't warrant flagging.
    if admin_count > 2:
        raw_score += 0.25
    ...
    # Velocity spike: >10 events/min sustained across the session window
    if event_velocity > 10:
        raw_score += 0.20
        factors.append("high event velocity spike")

    score = round(min(raw_score, 1.0), 3)
    user = events[0].user or "unknown"
    return UserAnomalyScore(
        user=user,
        anomaly_score=score,
...
def score_all_users(events: list[ForensicEvent]) -> list[UserAnomalyScore]:
    """
    Score each user's session against the trained baseline.
