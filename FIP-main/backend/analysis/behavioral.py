        ...
        for hour_key, count in hour_counts.items():
            ...
            if z > ZSCORE_THRESHOLD:
                anomalies.append({
                    ...
                    'user': user,
                    'description': (
                        f"User {user!r} generated {count} events in hour {hour_key} "
    ...
    for user, ev_list in user_events.items():
        ...
        if max_hosts >= VELOCITY_THRESHOLD and max_info:
            ...
            anomalies.append({
                ...
                'user': user,
                'description': (
                    f"User {user!r} accessed {max_hosts} distinct hosts in "
    ...
    for e in priv_events:
        ...
        if not (WORK_HOUR_START <= hour < WORK_HOUR_END):
            anomalies.append({
                ...
                'user': e.user,
                'description': (
                    f"SeDebugPrivilege assigned to {e.user!r} at "
...
def analyze_behavior(events: list[ForensicEvent]) -> dict:
    ...
    Keys: anomalies, profiled_users, analysis_window_hours, highest_severity.
    Each anomaly: anomaly_type, user, description, z_score, threshold, observed, severity.
    """
    if not events:
        return {
            'anomalies': [],
    ...
    # Sort by severity (high first), then by user
    _sev = {'high': 0, 'medium': 1, 'low': 2}
    anomalies.sort(key=lambda a: (_sev.get(a['severity'], 3), a['user']))
