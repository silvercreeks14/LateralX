...
type Section =
  | 'ingest' | 'timeline' | 'graph' | 'analysis'
  | 'chat' | 'cases' | 'storyline' | 'ml-settings'
...
export default function App() {
  ...
  const [storylineReport,   setStorylineReport]   = useState<AttackStoryline | null>(null)
  ...
  const handleQuickScan = async (uploadId?: number | null) => {
    setQuickScanLoading(true); setError(null)
    try {
      const result = await api.mlQuickScan(activeCaseId, uploadId ?? null)
      setRca(result)
      switchSection('analysis')
    } catch (e: unknown) {
  ...
  const handleRunStoryline = async () => {
    setStorylineLoading(true)
    try {
      const s = await api.mlStoryline(activeCaseId, selectedGraphUploadId)
      setStorylineReport(s)
    } finally { setStorylineLoading(false) }
  }
      ...
      <aside className="w-56 flex-shrink-0 flex flex-col bg-slate-900 border-r border-slate-800">
        ...
        <div className="px-4 py-5 border-b border-slate-800 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-white font-bold tracking-tight">LateralX</span>
          </div>
        </div>
        ...
        <nav className="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto">
          ...
          <NavItem id="analysis"  label="Analysis"      active={activeSection === 'analysis'}  onClick={switchSection} icon={nav.analysis} />
          <NavItem id="chat"      label="Analyst Chat"  active={activeSection === 'chat'}      onClick={switchSection} icon={nav.chat} />
          ...
          <div className="p-6 max-w-6xl">
            ...
            {activeSection === 'analysis' && (
              <NarrativePanel
                ...
                quickScanLoading={quickScanLoading}
            ...
            {activeSection === 'storyline' && (
              <StorylinePanel
                activeCaseId={activeCaseId}
                selectedUploadId={selectedGraphUploadId}
                storyline={storylineReport}
                loading={storylineLoading}
                onRun={handleRunStoryline}
              />
            )}
