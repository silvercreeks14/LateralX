...
const SOURCE_COLORS = [
  ...
  'bg-fuchsia-100 dark:bg-fuchsia-900/40 text-fuchsia-700 dark:text-fuchsia-400',
...
function SourceBadge({ event }: { event: ForensicEvent }) {
  ...
  return (
    <span className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-semibold
      ${isKnown ? 'bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-500'}`}>
...
export default function Timeline({ events, uploads = [] }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
      {/* Header / legend */}
      <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between flex-wrap gap-2">
        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{events.length} events</span>
      ...
      {/* Source legend */}
      {multiSource && (
        <div className="px-4 py-2 border-b border-slate-200 dark:border-slate-800 bg-slate-100/50 dark:bg-slate-800/50 flex flex-wrap gap-2">
          {uploads.map((u, idx) => {
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60">
            {events.map(event => {
              ...
              return (
                <tr key={event.id} className={`${rowHighlight(event)} hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors`}>
                  <td className="px-3 py-2 font-mono text-xs text-slate-500 whitespace-nowrap">
                    {event.timestamp.replace('T', ' ').slice(0, 19)}
                  </td>
                  <td className="px-3 py-2 text-xs text-slate-700 dark:text-slate-300 whitespace-nowrap font-medium">
                    {event.source_host}
                  </td>
                  <td className="px-3 py-2 text-xs text-slate-600 dark:text-slate-400 whitespace-nowrap">
                    {event.user ?? <span className="text-slate-400 dark:text-slate-700">—</span>}
                  </td>
                  <td className="px-3 py-2 text-xs text-slate-600 dark:text-slate-400 whitespace-nowrap max-w-[160px]">
                  ...
                  <td className="px-3 py-2 text-xs text-slate-600 dark:text-slate-400 max-w-xs truncate" title={event.description}>
