...
const MITRE_MATRIX = [
  {
    tactic: 'Recon',
    bg: 'bg-cyan-950/30', border: 'border-cyan-800/30', text: 'text-cyan-400',
    hitBg: 'bg-cyan-700/60', hitText: 'text-cyan-100',
    techniques: [
  ...
  },
  {
    ...
    bg: 'bg-purple-950/30', border: 'border-purple-800/30', text: 'text-purple-400',
    hitBg: 'bg-purple-700/60', hitText: 'text-purple-100',
    techniques: [
  ...
  },
  {
    ...
    bg: 'bg-orange-950/30', border: 'border-orange-800/30', text: 'text-orange-400',
    hitBg: 'bg-orange-700/60', hitText: 'text-orange-100',
    techniques: [
  ...
  },
  {
    ...
    bg: 'bg-blue-950/30', border: 'border-blue-800/30', text: 'text-blue-400',
    hitBg: 'bg-blue-700/60', hitText: 'text-blue-100',
    techniques: [
  ...
  {
    tactic: 'Persistence',
    bg: 'bg-yellow-950/30', border: 'border-yellow-800/30', text: 'text-yellow-400',
    hitBg: 'bg-yellow-700/60', hitText: 'text-yellow-100',
    techniques: [
  ...
  },
  {
    ...
    bg: 'bg-slate-800/40', border: 'border-slate-700/40', text: 'text-slate-400',
    hitBg: 'bg-slate-600/60', hitText: 'text-slate-100',
    techniques: [
  ...
  {
    tactic: 'Impact',
    bg: 'bg-red-950/30', border: 'border-red-800/30', text: 'text-red-400',
    hitBg: 'bg-red-700/60', hitText: 'text-red-100',
    techniques: [
...
type TabId = 'detection' | 'timeline' | 'entities' | 'threat-map' | 'mitre-heatmap'
...
export default function ADIntelPanel({ activeCaseId, uploads }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-slate-900 dark:text-white font-semibold text-lg">AD Intelligence</h2>
      ...
      {/* Tab strip */}
      <div className="flex gap-1 bg-slate-100 dark:bg-slate-100/60 dark:bg-slate-800/60 rounded-xl p-1 flex-wrap">
        {TABS.map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className="flex-1 min-w-[72px] px-3 py-1.5 rounded-lg text-xs font-semibold transition-all whitespace-nowrap"
                      ...
                      <div
                        ...
                        className={`rounded px-2 py-1.5 flex items-center gap-2 ${
                          hit
                            ...
                            : 'bg-slate-100 dark:bg-slate-800/50 text-slate-500 dark:text-slate-600'
                        }`}
                      >
