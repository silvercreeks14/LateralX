...
const CY_STYLE: cytoscape.Stylesheet[] = [
  {
    ...
    style: {
      ...
      'font-family':              'system-ui, sans-serif',
      color:                      '#374151',
      'text-max-width':           '90px' as unknown as number,
    },
  },
  {
    selector: 'node[nodeType = "host"]',
  ...
  {
    selector: 'edge[edgeStyle = "dashed"]',
    style: { 'line-style': 'dashed' as const, 'line-dash-pattern': [6, 3] as unknown as number },
  },
  {
    selector: 'edge[edgeType = "threat"]',
    style: { 'line-color': '#fb923c', 'target-arrow-color': '#fb923c' },
  ...
  },
  {
    ...
    style: {
      ...
      'target-arrow-color': '#cbd5e1',
      'line-style':         'dashed' as const,
      'line-dash-pattern':  [4, 4] as unknown as number,
    },
  },
]
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            <h2 className="text-slate-900 dark:text-white font-semibold text-lg">AD Threat Map</h2>
          ...
          <button
            onClick={run}
            disabled={loading}
            className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold disabled:opacity-50 flex-shrink-0"
            style={{ background: loading ? '#334155' : '#00F0FF', color: loading ? '#94a3b8' : '#0f172a' }}
          >
            {loading ? (
              <>
        ...
        {(uploads ?? []).length > 1 && (
          <div className="flex items-center gap-2 mt-4 flex-wrap">
            ...
            {(uploads ?? []).map(u => (
              <button key={u.id} onClick={() => setSelectedUploadId(u.id)}
                className="px-2.5 py-1 rounded-full text-xs font-medium truncate max-w-[180px]"
      ...
      {result && (
        <>
          ...
          {/* Cytoscape canvas */}
          <div className="rounded-2xl overflow-hidden relative border border-slate-200 dark:border-slate-700" style={{ background: '#f1f5f9' }}>

            {/* Legend */}
            <div className="absolute top-3 left-3 z-10 bg-white/95 backdrop-blur rounded-lg shadow-sm px-3 py-2.5 text-xs space-y-1.5" style={{ border: '1px solid #e2e8f0' }}>
              <p className="font-semibold text-slate-500 uppercase tracking-wide text-[10px] mb-2">Node Types</p>
              <LegendRow color="#3b82f6" border="#1d4ed8" label="Host / Hostname" />
              <LegendRow color="#6366f1" border="#4338ca" label="User Account" />
              <LegendRow color="#1d4ed8" border="#1e3a8a" label="Pivot (most connected)" />
              <LegendRow color="#f97316" border="#c2410c" label="Threat / Attack (risk ≥ 70)" />
            ...
            {/* Controls hint */}
            <div className="absolute bottom-3 right-3 z-10 bg-white/90 rounded-md px-2 py-1 text-[10px] text-slate-400 pointer-events-none">
              click node to focus · double-click canvas to reset
            </div>
