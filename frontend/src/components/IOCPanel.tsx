...
const TYPE_STYLE: Record<string, string> = {
  ip:           'bg-red-50 dark:bg-red-950/20 text-red-700 dark:text-red-400',
  url:          'bg-orange-50 dark:bg-orange-950/20 text-orange-700 dark:text-orange-400',
  domain:       'bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-400',
  file_path:    'bg-blue-50 dark:bg-blue-950/20 text-blue-700 dark:text-blue-400',
  md5:          'bg-purple-50 dark:bg-purple-950/20 text-purple-700 dark:text-purple-400',
  sha256:       'bg-violet-50 dark:bg-violet-950/20 text-violet-700 dark:text-violet-400',
  registry_key: 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400',
}
...
function AbuseBadge({ score }: { score: number }) {
  const color =
    score >= 75 ? 'bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400' :
    score >= 30 ? 'bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400' :
                  'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400'
  return (
    <span
...
export default function IOCPanel({ iocs, tiStatus = null }: Props) {
      ...
      {hasEnrichment ? (
        <div className={`rounded-lg border p-3 ${
          ...
          <div className="flex items-center gap-2 mb-2">
            <svg className={`w-4 h-4 flex-shrink-0 ${flagged.length > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}
              fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M2.166 4.999A11.954 11.954 0 0010 1.944 11.954 11.954 0 0017.834 5c.11.65.166 1.32.166 2.001 0 5.225-3.34 9.67-8 11.317C5.34 16.67 2 12.225 2 7c0-.682.057-1.35.166-2.001zm11.541 3.708a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
            </svg>
            <span className={`text-xs font-semibold ${flagged.length > 0 ? 'text-red-700 dark:text-red-400' : 'text-green-700 dark:text-green-400'}`}>
          ...
          {flagged.length > 0 && (
            <div className="space-y-1">
              {flagged.map(({ ioc, ti }, idx) => (
                <div key={idx} className="flex items-center gap-2 text-xs bg-white dark:bg-slate-800 border border-red-200 dark:border-red-900/30 rounded px-2.5 py-1.5">
                  <span className={`px-1.5 py-0.5 rounded font-medium text-xs ${TYPE_STYLE[ioc.type] ?? 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400'}`}>
                    {ioc.type}
                  </span>
                  <span className="font-mono text-slate-700 dark:text-slate-300 truncate flex-1" title={ioc.value}>{ioc.value}</span>
        ...
        <div className="rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 px-3 py-2 flex items-start gap-2">
          ...
          <p className="text-xs text-slate-500">
            {tiStatus && !tiStatus.vt_configured && !tiStatus.abuseipdb_configured ? (
              <>No API keys configured — add <code className="bg-slate-100 dark:bg-slate-900 px-1 rounded border border-slate-300 dark:border-slate-700">VT_API_KEY</code> and <code className="bg-slate-100 dark:bg-slate-900 px-1 rounded border border-slate-300 dark:border-slate-700">ABUSEIPDB_KEY</code> to <code className="bg-slate-100 dark:bg-slate-900 px-1 rounded border border-slate-300 dark:border-slate-700">.env</code> and restart the server.</>
            ) : tiStatus && (!tiStatus.vt_configured || !tiStatus.abuseipdb_configured) ? (
              <>Partial configuration — {!tiStatus.vt_configured && <><code className="bg-slate-100 dark:bg-slate-900 px-1 rounded border border-slate-300 dark:border-slate-700">VT_API_KEY</code> missing</>}{!tiStatus.vt_configured && !tiStatus.abuseipdb_configured && ' and '}{!tiStatus.abuseipdb_configured && <><code className="bg-slate-100 dark:bg-slate-900 px-1 rounded border border-slate-300 dark:border-slate-700">ABUSEIPDB_KEY</code> missing</>}. No IP or domain IOCs were found in this dataset, or the API returned no results.</>
            ) : (
        ...
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
            {parsed.map(({ ioc, ti }, idx) => {
              ...
              return (
                <tr key={idx} className={isFlagged ? 'bg-red-50 dark:bg-red-950/20' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50'}>
                  <td className="px-2 py-1.5">
                    <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${TYPE_STYLE[ioc.type] ?? 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
                      {ioc.type}
                    </span>
                  </td>
                  <td className="px-2 py-1.5 font-mono text-slate-700 dark:text-slate-300 max-w-[200px] truncate" title={ioc.value}>
                    {ioc.value}
                  </td>
