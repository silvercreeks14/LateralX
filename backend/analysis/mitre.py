...
def _guard_t1547001(event, combined: str) -> bool:
    # EID 4656/4658/4663/4690: handle-request/close events for Winlogon registry key paths
    # are generated by SYSTEM during normal profile loads — not attacker persistence writes.
    if event.event_id in ("4656", "4658", "4663", "4690", "800"):
        return False
...
def _guard_no_eid800(event, combined: str) -> bool:
    # EID 800 = PowerShell "Pipeline execution details" — these events expose the internal
    # source of attack frameworks (Empire, Covenant) including VirtualAllocEx, Invoke-Command,
    # and encoded-command flags that are part of the framework's own loader, not an attacker
    # command typed at a terminal. Suppressing on EID 800 eliminates a large class of FPs
    # while preserving detection on EID 4688 (process creation) and 4103/4104 (script block).
    return event.event_id != "800"


def _guard_t1059001(event, combined: str) -> bool:
    # EID 800 pipeline events contain internal framework encoded flags — suppress there.
    # Allow on all other EIDs (4688 process creation, 4103/4104 ScriptBlock logging, etc.).
    return event.event_id != "800"
...
_GUARDS: dict[str, object] = {
    ...
    "T1003.002": _guard_no_eid800,   # SAM dump keywords in EID 800 = pipeline noise
    "T1003.003": _guard_no_eid800,   # NTDS keywords in EID 800 = pipeline noise
    "T1021.002": _guard_t1021002,
    "T1021.006": _guard_no_eid800,   # Invoke-Command in EID 800 = framework internals
    "T1047":     _guard_t1047,
    "T1055":     _guard_no_eid800,   # VirtualAllocEx in EID 800 = framework loader code
    "T1059.001": _guard_t1059001,    # PowerShell keywords in EID 800 = framework internals
    "T1027":     _guard_t1027,
    "T1082":     _guard_t1082,
...
_MAP: list[tuple[list[str], MitreTechnique]] = [
    ...
    # "cmd.exe" bare removed — appears in every process creation message as the image
    # name or parent path; requires execution context (cmd /c) to signal attacker use.
    ...
    # events are normal on any Windows host; DDoS requires correlated volume, not one event.
    ...
    # "an operation was performed on an object" removed — generic EID 4662 description
    # that fires on any object access, not specifically DS-Replication. Only the
    # GUID 1131f6aa (DS-Replication-Get-Changes-All) unambiguously identifies DCSync.
    (["dcsync", "ds-replication", "1131f6aa", "lsadump::dcsync", "getchangesall",
      "drsuapi", "ds-replication-get-changes"],
     MitreTechnique(id="T1003.006", name="DCSync", tactic="Credential Access")),

    # SAM credential dump — reg.exe save of HKLM\SAM or Mimikatz lsadump::sam
    (["reg save hklm\\sam", "reg save hklm\\system", "lsadump::sam",
      "sam hive", "hklm\\sam", "hashdump", "empire_powerdump",
      "secretsdump /sam", "reg save sam"],
     MitreTechnique(id="T1003.002", name="Security Account Manager",
                    tactic="Credential Access")),

    # NTDS credential dump — ntdsutil IFM extraction or VSS shadow + ntds.dit copy
    (["ntdsutil", "ntds.dit", "activate instance ntds", "install from media",
      "vssadmin create shadow", "copy.*ntds", "ntds exfil",
      "globalroot.*device.*harddisk.*ntds"],
     MitreTechnique(id="T1003.003", name="NTDS", tactic="Credential Access")),
