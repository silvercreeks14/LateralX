...
# Tuned for speed: 80-event cap + 45-event windows yields ≤2 windows → max 3 LLM
# calls (2 windows + 1 synthesis), down from 5 with the previous 150/50 settings.
WINDOW_SIZE                = 45
WINDOW_OVERLAP             = 5
MAX_EVENTS_BEFORE_SAMPLING = 80
...
def _build_deterministic_narrative(
    ...
    """Comprehensive summary from deterministic analysis — no LLM dependency."""
    sev_label = (
    ...
    parts = [f"Deterministic forensic analysis of {event_count} events."]
    if suspicious_users:
    ...
    if techniques:
        remediation_techs = [t.name for t in techniques[:3]]
        parts.append(
            f"Priority remediation focus: {', '.join(remediation_techs)}. "
            "Review MITRE Threat Intel tab for per-technique detection rules and mitigations."
        )
    return " ".join(parts)
...
def _build_deterministic_result(
    ...
    """
    Comprehensive forensic result derived entirely from deterministic analysis.
    Populates patient zero, initial access vector, and pivot chain from graph data.
    Used when LLM_PROVIDER=none or when Ollama is unreachable.
    """
    ...
    if initial_access_techs:
        ...
        initial_access_vector = f"Earliest detected technique: {techniques[0].name} ({techniques[0].id})"
    else:
        initial_access_vector = ""

    pivot_chain: list[str] = []
    if suspicious_users:
        for user in suspicious_users[:8]:
            pivot_chain.append(
                f"{user} — lateral movement detected (accessed ≥3 hosts within 30 min)"
            )
...
def run_full_analysis(events: list[ForensicEvent]) -> RCAResult:
    ...
    Always runs the full deterministic pipeline (MITRE, IOCs, ML, correlation).
    When LLM_PROVIDER=ollama and Ollama is reachable, also runs windowed LLM
    inference to produce an AI-authored investigation narrative.
    Falls back to the comprehensive deterministic result on any LLM error.
    """
    if not events:
    ...
    # ── Ollama-assisted analysis (with graceful fallback) ─────────────────────
    try:
