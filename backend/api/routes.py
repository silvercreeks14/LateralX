...
def analyze_ad_rules(
    ...
    """
    Run 77 rule-based AD detection rules against the selected events.
    Covers Kerberoasting, DCSync, Lateral Movement, Privilege Escalation,
    Persistence, Reconnaissance, Credential Dump, Network, and Tool Detection
    categories. Also runs attack chain correlation and tool signature fingerprinting.
    """
...
@router.post("/analyze/lmd-rf", summary="LMD Random Forest scan and attack graph")
async def analyze_lmd_rf_upload(
    file: UploadFile = File(...),
    db: Session = Depends(get_db),
    current_user: UserModel = Depends(get_current_user),
):
    """
    Parse an uploaded LMD-labelled Sysmon CSV, run the trained RF model, and
    write attack_graph.html for optional download.
    """
    if not file.filename:
        raise HTTPException(400, "No filename provided.")
    if os.path.splitext(file.filename)[1].lower() != ".csv":
        raise HTTPException(400, "LMD RF scan expects a .csv file.")
    ...
    try:
        events = detect_and_parse(file.filename, content, parser_hint="lmd")
    except ValueError as exc:
        raise HTTPException(400, str(exc))
    if not events:
        raise HTTPException(400, "No LMD events parsed. Check the CSV headers and Label column.")

    from backend.analysis.lmd_model import DEFAULT_GRAPH_PATH, run_lmd_model_and_graph

    try:
        anomalies, graph_data = await run_in_threadpool(
            run_lmd_model_and_graph,
            events,
            DEFAULT_GRAPH_PATH,
        )
    except Exception as exc:
        raise HTTPException(500, f"LMD RF scan failed: {exc}")

    result = {
        "filename": file.filename,
        "events_parsed": len(events),
        "anomalies": anomalies,
        "anomaly_count": len([a for a in anomalies if not a.startswith("Error:")]),
        "graph_data": graph_data,
        "graph_download_url": "/api/analyze/lmd-rf/attack-graph",
    }
    _write_audit(db, "lmd_rf_scan", {
        "filename": file.filename,
        "events_parsed": len(events),
...
@router.get("/analyze/lmd-rf/attack-graph", summary="Download generated LMD attack graph")
def download_lmd_attack_graph(
    current_user: UserModel = Depends(get_current_user),
):
    from backend.analysis.lmd_model import DEFAULT_GRAPH_PATH

    if not DEFAULT_GRAPH_PATH.exists():
        raise HTTPException(404, "No LMD attack graph has been generated yet.")
    return FileResponse(
        DEFAULT_GRAPH_PATH,
        media_type="text/html",
        filename="attack_graph.html",
    )
...
def get_case_report(
    ...
    # ── Compute ML stats and tool config for the tool registry section ─────────
    import sys as _sys
    try:
        import sklearn as _sk
        sklearn_ver = _sk.__version__
    except Exception:
        sklearn_ver = "unknown"

    gt_rows = db.query(MLGroundTruthModel).all()
    _ml_stats: MLStats | None = None
    if gt_rows:
        _tp = _tn = _fp = _fn = 0
        for _r in gt_rows:
            _pos = _r.predicted_risk in ("suspicious", "high_risk")
            if _r.is_true_positive and _pos:       _tp += 1
            elif not _r.is_true_positive and not _pos: _tn += 1
            elif not _r.is_true_positive and _pos:    _fp += 1
            elif _r.is_true_positive and not _pos:     _fn += 1
        _prec = _tp / (_tp + _fp) if (_tp + _fp) > 0 else None
        _rec  = _tp / (_tp + _fn) if (_tp + _fn) > 0 else None
        _f1   = (2 * _prec * _rec / (_prec + _rec)
                 if _prec is not None and _rec is not None and (_prec + _rec) > 0 else None)
        _acc  = (_tp + _tn) / len(gt_rows) if gt_rows else None
        _ml_stats = MLStats(
            total_verifications=len(gt_rows),
            true_positives=_tp, true_negatives=_tn, false_positives=_fp, false_negatives=_fn,
            precision=round(_prec, 4) if _prec is not None else None,
            recall=round(_rec, 4) if _rec is not None else None,
            f1_score=round(_f1, 4) if _f1 is not None else None,
            accuracy=round(_acc, 4) if _acc is not None else None,
            note=f"Based on {len(gt_rows)} analyst verification(s).",
        )

    _model_status = _ml_mod.get_model_status()
    _tool_config = {
        "Platform": "LateralX Forensic Intelligence Platform (FIP)",
        "Python Version": _sys.version.split()[0],
        "scikit-learn Version": sklearn_ver,
        "Analysis Mode": (
            f"AI-assisted (LLM) — {result.windows_analyzed} analysis window(s)"
            if result.windows_analyzed > 0 else "Deterministic Quick Scan (no LLM)"
        ),
        "ML Model Status": "Trained" if _model_status.get("trained") else "Not trained (heuristic fallback)",
        "ML Training Date": _model_status.get("trained_at") or "N/A",
        "ML Training Events": str(_model_status.get("trained_on_events", 0)),
        "ML Training Entities": str(_model_status.get("trained_on_users", 0)),
        "Report Generated At (UTC)": datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"),
    }

    html = generate_case_report(case_obj, result, events, notes, uploads,
                                ml_stats=_ml_stats, tool_config=_tool_config)
...
@router.get("/benchmark", response_model=BenchmarkReport,
            summary="Run model quality benchmark against OTRF/MITRE ground-truth dataset")
